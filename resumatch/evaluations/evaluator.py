...
def evaluate_resume(resume_path):
    ...
    applicant_info = {
        'name': extract_name(doc),  # Enhanced name extraction
...
# Enhanced name extraction using spaCy with fallback regex
def extract_name(doc):
    # Step 1: Use spaCy's NER to detect the first 'PERSON' entity
    for ent in doc.ents:
        if ent.label_ == 'PERSON':
            return ent.text
    
    # Step 2: Fallback - Use regex to detect common name patterns if NER fails
    # Assuming typical names are in the format: First Last or First Middle Last
    name_pattern = r'\b([A-Z][a-z]+(?:\s+[A-Z][a-z]+)+)\b'  # Detect capitalized name patterns
    text = doc.text
    ...
    # Step 3: Return default if no name is found
    return 'Applicant'
...
# Enhanced experience extraction with date range arithmetic
